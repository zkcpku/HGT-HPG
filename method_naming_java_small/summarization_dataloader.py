...
class SingeJsonDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        graph_dict = deserialize_graph_dicts(origin_data['graph_dict'])
        target_sent = origin_data['tgt']
        edge_dict = graph_dict[1]
        each_node_dict = graph_dict[2]
        # input_graph = dict2graph(graph_dict[1], graph_dict[2], self.zero_dict)
        node_info = graph_dict[0]

        # return (node_info, input_graph, target_sent)
        return (node_info, edge_dict, each_node_dict, target_sent)
...
def get_collate_fn(src_vocab, zero_dict, count_node_nums_type = 'subtoken', max_tgt_len = 150, device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
    def collate_fn(data):
        # len(data) == batch_size
        node_info, edge_dict, each_node_dict, target_sent = zip(*data)
        bs = len(node_info)
        input_graphs = []
        all_subtoken_nums = []
        for i in range(bs):
        ...
        target_sent = [e[:max_tgt_len] for e in target_sent]
        return batch_graph, target_sent, all_subtoken_nums
    return collate_fn

def unit_test():
    import psutil
    import time
    zero_dict_path = '/home/username/workspace/HGT-DGL/data/summarization/python_wan/no_share_subtoken/step2/zero_dict'
    zero_dict = pickle.load(open(zero_dict_path, 'rb'))
    src_vocab_path = '/home/username/workspace/HGT-DGL/data/summarization/python_wan/no_share_subtoken/step2/src_vocab.pkl'
    src_vocab = pickle.load(open(src_vocab_path, 'rb'))
    paths = [
            # '/home/username/workspace/HGT-DGL/data/codexglue/python/subtoken_share/step3/train_0to99999.json',
            #  '/home/username/workspace/HGT-DGL/data/codexglue/python/subtoken_share/step3/train_100000to199999.json',
            #  '/home/username/workspace/HGT-DGL/data/codexglue/python/subtoken_share/step3/train_200000to251820.json',
        '/home/username/workspace/HGT-DGL/data/summarization/python_wan/no_share_subtoken/step3/valid_0to18505.json']
    t1 = time.time()
    ...
    collate_fn = get_collate_fn(src_vocab, zero_dict,max_tgt_len=5)
    ...
    for batch_idx, sample in enumerate(dataloader):
        batch_graph, target_sent, all_subtoken_nums = sample
