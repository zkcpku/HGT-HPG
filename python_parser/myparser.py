...
# asdl_text = open(os.path.join(os.path.dirname(__file__), 'py39_asdl.txt')).read()
grammar = ASDLGrammar.from_text(asdl_text)
mod_type = ASDLType('mod').name
identifier_type = ASDLType('identifier').name
...
def to_dict(node, add_list, parent_field = 'body'):
    if type(node) == type(" "):
        ...
        return
    this_dict = {'name': node.production.constructor.name,'type':node.production.type.name,'children':[], 'toParentField':parent_field}
